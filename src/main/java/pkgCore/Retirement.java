...
public class Retirement {
	...
	public double MonthlySavings() {
		double interestPerMonth = dAnnualReturnWorking * 0.083333333333;
		double pmt = (12*(dRequiredIncome-dMonthlySSI)*((1-Math.pow(1+dAnnualReturnRetired,-1*iYearsRetired)/dAnnualReturnRetired))) / (Math.pow(1+interestPerMonth,12*iYearsToWork)-1)*interestPerMonth;
		
		return pmt;
	}

	public double TotalAmountToSave() {
		double pv = 12*(dRequiredIncome-dMonthlySSI)*((1-Math.pow(1+dAnnualReturnRetired,-1*iYearsRetired)/dAnnualReturnRetired));
